let handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    if(user === conn.user.jid) return conn.reply(m.chat, `*Ehh Gila kau ya, Masa pacaran ama gw ngotak lah.*`, m)
    ...
    if(global.db.data.users[user].pasangan != m.sender){
      ...
      conn.reply(m.chat,`*Anda baru saja telah menolak @${user.split('@')[0]} Kasian awokaowk mmpus*`,m,{contextInfo: {
        mentionedJid: [user]
      }})
    }
	}	
